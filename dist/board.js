...
Board.prototype.play = function (i, j) {
    ...
    var self = this;
    _underscore2.default.each(neighbors, function (n) {
    ...
    // detect suicide
    if (_underscore2.default.isEmpty(captured) && this.get_group(i, j)["liberties"] === 0) {
        this.board[i][j] = Board.EMPTY;
        this.attempted_suicide = true;
        return false;
    }

    _underscore2.default.each(captured, function (group) {
        _underscore2.default.each(group["stones"], function (stone) {
...
Board.prototype.get_group = function (i, j) {
    ...
    var _loop = function _loop() {
        var stone = queue.pop();
        if (visited[stone]) return "continue";

        var neighbors = _this.get_adjacent_intersections(stone[0], stone[1]);
        var self = _this;
